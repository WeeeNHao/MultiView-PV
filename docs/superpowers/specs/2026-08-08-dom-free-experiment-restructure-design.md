...
8. 原表 3 的四行数据不浪费，改造成"为什么不用正射"的分析表（新表 8），复用现成结果 + 补一轮 `fb_tdom_only` t=2。
9. **`proj_affine` 不单独调参**：跑出什么记录什么。为它单独调阈值会破坏与另外两种投影方式的可比性。
...
| 表 8 | 分析：正射路径为何是死路 | `dom` / `fb_tdom_only` / `ours` / `full` | **是，1 轮**（`fb_tdom_only` t=2） |
...
| 仅作反馈源（只 re-prompt TDOM） | `fb_tdom_only` | **需补 t=2** |
...
三站并行 → **关键路径 CangFang ≈ 3.7 天**。BeiOu / XinXie 约 1 天后腾出 GPU0 / GPU1，而 `abl_*` 与 `proj_*` 各轮彼此独立、可迁移过去分担，实际预期 **2.5–3 天**。
...
- 新增新表 8（正射路径分析），变体 `dom` / `fb_tdom_only` / `ours` / `full`，t=2。
...
- [ ] `abl_*` ×6、`d1..d5` ×5、`fb_tdom_only` 三站均补齐 t=2。
