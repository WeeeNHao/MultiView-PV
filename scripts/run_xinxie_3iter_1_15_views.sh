...
OUTPUT_ROOT="${OUTPUT_ROOT:-/data/dataset/PV/ZS_PV/003-XinXie-0518}"
...
prepare_iter0_cache() {
	if [[ "$SKIP_EXISTING" == "1" ]] && compgen -G "${ITER0_PROJ_DIR}/*.shp" > /dev/null; then
		echo "[xinxie] reuse iter0 cache: ${ITER0_PROJ_DIR}"
		return
	fi
...
run_iter0_postprocess() {
	...
	if [[ "$SKIP_EXISTING" == "1" && -f "$final_shp" ]]; then
		echo "[xinxie] iter=0 view=${view_num} exists, skip"
		return
	fi
	...
	echo "[xinxie] iter=0 view=${view_num} postprocess"
	run_pipeline "${opts[@]}"
}
...
run_iter_with_prompt() {
	...
	if [[ "$SKIP_EXISTING" == "1" && -f "$final_shp" ]]; then
		echo "[xinxie] iter=${iter_idx} view=${view_num} exists, skip"
		return
	fi
	...
	echo "[xinxie] iter=${iter_idx} view=${view_num}"
	run_pipeline "${opts[@]}"
}
